...
export default function ApiClientPage() {
    ...
    const getMethodColor = (methodName: string) => {
        switch (methodName) {
            ...
            default:
                return 'text-[#10162F] dark:text-white';
        }
    };
                ...
                <div className="bg-white dark:bg-[#10162F] rounded-lg shadow-xl border-2 border-[#10162F] dark:border-gray-700 overflow-hidden">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="relative group">
                                <select
                                    ...
                                    className={`appearance-none bg-gradient-to-r from-white to-gray-50 dark:from-[#1D233D] dark:to-[#243055] border-2 border-accent-yellow dark:border-gray-600 rounded-lg px-4 py-3 pr-12 text-sm font-bold ${getMethodColor(method)} focus:outline-none focus:ring-2 focus:ring-accent-yellow dark:focus:ring-[#FFD300] shadow-lg hover:shadow-xl transition-all duration-200 cursor-pointer min-w-[120px] hover:border-acc dark:hover:border-[#FFD300]`}
                                >
                                    <option value="GET" className="bg-white dark:bg-[#1D233D] text-green-500 dark:text-green-400 font-bold">GET</option>
                                    <option value="POST" className="bg-white dark:bg-[#1D233D] text-blue-500 dark:text-blue-400 font-bold">POST</option>
                                    <option value="PUT" className="bg-white dark:bg-[#1D233D] text-yellow-500 dark:text-yellow-400 font-bold">PUT</option>
                                    <option value="PATCH" className="bg-white dark:bg-[#1D233D] text-purple-500 dark:text-purple-400 font-bold">PATCH</option>
                                    <option value="DELETE" className="bg-white dark:bg-[#1D233D] text-red-500 dark:text-red-400 font-bold">DELETE</option>
                                </select>
                                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text- dark:text-white group-hover:text-[#3A10E5] dark:group-hover:text-[#FFD300] transition-colors duration-200">
                                    <svg className="fill-current h-5 w-5 transform group-hover:rotate-180 transition-transform duration-200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                                        <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                                    </svg>
                                </div>
                                <div className="absolute inset-0 rounded-lg bg-gradient-to-r from-[#3A10E5]/10 to-[#5B21B6]/10 dark:from-[#FFD300]/10 dark:to-[#FFC700]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none"></div>
                            </div>
                            ...
                            <input
                                ...
                                className="flex-1 bg-white dark:bg-[#1D233D] border-2 border-[#10162F] dark:border-gray-600 rounded-lg px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-accent-yellow text-[#10162F] dark:text-white shadow-lg hover:shadow-xl transition-all min-w-0"
                            />
                            ...
                            >
                                ...
                                {isLoading ? <Loader2 size={16} className="animate-spin relative z-10" /> : <Play size={16} fill="currentColor" className="relative z-10" />}
                    ...
                    <div className="grid lg:grid-cols-2 gap-6 mt-6">
                                ...
                                {["params", "headers", "body"].map((tab) => (
                                    <button
                                        ...
                                        className={`px-6 py-4 text-sm font-bold capitalize border-r-2 border-[#10162F] dark:border-gray-600 transition-all relative ${activeTab === tab
                                            ? "bg-white dark:bg-[#243055] text-accent-yellow dark:text-[#FFD300] shadow-lg"
                                            : "bg-transparent text-[#10162F] dark:text-gray-400 hover:bg-white/70 dark:hover:bg-[#243055]/50"
                                            }`}
                                    >
                                        {tab}
                                        {activeTab === tab && (
                                            <div className="absolute bottom-0 left-0 right-0 h-1 bg-accent-yellow dark:bg-[#FFD300]"></div>
                                        )}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="bg-white dark:bg-[#1a2040] mr-6 rounded-lg border-2 border-[#10162F] dark:border-gray-600 overflow-hidden shadow-lg">
                            {response ? (
                                <>
<div className="border-b border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-[#1a2040]">
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`inline-flex items-center px-3 rounded-md text-sm font-semibold ${response.status >= 200 && response.status < 300 ? 'text-green-800 dark:text-green-200 bg-green-100 dark:bg-green-900/20' :
                                                    response.status >= 400 ? 'text-red-800 dark:text-red-200 bg-red-50 dark:bg-red-900/20' : 'text-yellow-800 dark:text-yellow-200 bg-yellow-50 dark:bg-yellow-900/20'
                                                    }`}>
                                                    <div className={`w-2 h-2 rounded-full mr-2 ${response.status >= 200 && response.status < 300 ? 'bg-green-500' :
                                    ...
                                    {isLoading ? (
                                        <div className="flex flex-col items-center gap-6">
                                            <div className="relative">
                                                <div className="w-16 h-16 border-4 border-accent-yellow dark:border-[#FFD300] border-t-transparent rounded-full animate-spin"></div>
                                                <div className="absolute inset-0 w-16 h-16 border-4 border-transparent border-b-accent-yellow/30 dark:border-b-[#FFD300]/30 rounded-full animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }}></div>
                                            </div>
                                            <div className="text-center">
                                                <p className="font-bold text-lg tracking-wide text-accent-yellow dark:text-[#FFD300]">SENDING REQUEST</p>
                                        ...
                                        <div className="flex flex-col items-center gap-6 opacity-60">
                                            <div className="w-24 h-24 dark:from-[#1D233D] dark:to-[#243055] dark:border-gray-600 rounded-xl flex items-center justify-center">
                                                <Globe size={70} className="text-accent-yellow dark:text-[#FFD300]" />
                                            </div>
                                            <div className="text-center">
...
function ParamsEditor({
    ...
    return (
        <div className="p-4 bg-gradient-to-br from-white to-gray-50 dark:from-[#1D233D] dark:to-[#243055] min-h-full">
            <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-200 dark:border-gray-600">
                <div className="flex items-center gap-3">
                    <div>
                        <h3 className="text-lg font-bold text-[#10162F] dark:text-white">{title}</h3>
                        <p className="text-xs text-gray-500 dark:text-gray-400">Configure your request {title.toLowerCase()}</p>
                    </div>
                </div>
                <button
                    onClick={onAdd}
                    className="group relative flex items-center gap-2 px-4 py-1.5 bg-accent-yellow dark:from-[#FFD300] dark:to-[#FFC700] text-white dark:text-[#10162F] transform transition-all hover:cursor-pointer hover:opacity-75 duration-200 text-sm font-bold overflow-hidden"
                >
                    <Plus size={16} strokeWidth={2.5} className="relative z-10" />
                    <span className="relative z-10">Add {title.slice(0, -1)}</span>
                </button>
            </div>
            ...
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                {items.map((item, index) => (
                    <div key={item.id} className="group relative">
                        <div className="flex gap-2 items-start py-1 dark:bg-[#1a2040] rounded-lg dark:border-gray-600 hover:border-[#3A10E5] dark:hover:border-[#FFD300] transition-all duration-200">
                            <div className="flex items-center justify-center w-8 h-8 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 rounded-md text-xs font-bold text-gray-600 dark:text-gray-300 shadow-inner">
                                {index + 1}
                            </div>

                            <div className="flex-1 space-y-2 sm:space-y-0 sm:flex sm:gap-2">
                                <div className="relative flex-1">
                                    <label className="absolute -top-1.5 left-2 bg-white dark:bg-[#1a2040] px-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 z-10">
                                        Key
                                    </label>
                                    <input
                                        type="text"
                                        ...
                                        className="w-full bg-gray-50 dark:bg-[#0f1419] border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 text-xs font-mono text-[#10162F] dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-[#3A10E5] dark:focus:ring-[#FFD300] focus:border-transparent focus:bg-white dark:focus:bg-[#1a2040] transition-all duration-200 placeholder-gray-400 dark:placeholder-gray-500"
                                    />
                                </div>
                            ...
                            <button
                                ...
                                className="p-1.5 text-gray-600 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-all duration-200 opacity-70 group-hover:opacity-100 transform scale-90 group-hover:scale-100"
                ...
                {items.length === 0 && (
                    <div className="text-center py-20 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl bg-white dark:bg-gray-800 transition-all duration-500 cursor-pointer group" onClick={onAdd}>
                        <div className="flex flex-col items-center gap-6">
                            
                            <div className="space-y-3 max-w-sm mx-auto">
                                <h3 className="text-md font-bold text-gray-700 dark:text-gray-200 group-hover:text-accent-yellow dark:group-hover:text-[#FFD300] transition-colors duration-300">
